...
// VersionInfo describes the structural parameters of a QR code version at a given
...
//nolint:funlen // init populates version capacity tables from QR spec data
func init() {
	raw := [][21]int{
		{
			26,
...
// GetVersionInfo returns the [VersionInfo] for the given version (1–40) and
// error correction level (0–3). Returns an error if version or ecLevel
// is out of range.
...
// DataCapacity returns the number of data codeword bytes available for the
// given version and error correction level. Returns 0 if version or ecLevel
// is invalid.
...
// MinVersionForData returns the smallest QR code version (1–40) that can
// hold dataLen bytes at the given error correction level. Returns an error
// if ecLevel is out of range, dataLen is negative, or the data exceeds the
// maximum capacity of version 40.
